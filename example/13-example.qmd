...
The US Federal Reserve provides thousands of economic datasets at [FRED](https://fred.stlouisfed.org/). We can use the [{fredr} R package](https://sboysel.github.io/fredr/) to access their servers and download the data directly into R.
...
To fix this, the {fredr} package gives you data in tidy (or long) form and only provides three columns:

```{r show_fred_head}
head(fred_raw)
```

The `series_id` column is the ID of the variable from FRED , `date` is… the date, and `value` is the value.

Right now, our `fred_raw` dataset has only 3 columns, but nearly 3,000 rows since the six indicators we got from the server are all stacked on top of each other. To actually work with these, we need to filter the raw data so that it only includes the indicators we're interested in. For instance, if we want to plot retail sales, we need to select only the rows where the `series_id` is `RSXFSN`. Make a smaller dataset with `filter()` to do that:

```{r make-retail-sales}
retail_sales <- fred_raw |> 
  filter(series_id == "RSXFSN")
...
fred_monthly_things <- fred_raw |> 
  filter(series_id %in% c("FPCPITOTLZGUSA", "UNRATE", "RSXFSN")) |> 
  # Convert the series_id column into multiple columns, using the "values" for values
...
fred_monthly_things <- fred_raw |> 
  filter(series_id %in% c("UNRATE", "RSXFSN")) |> 
  # Convert the series_id column into multiple columns, using the "values" for values
  pivot_wider(names_from = series_id, values_from = value) |> 
...
ggplot(gdp_only, aes(x = date, y = value)) +
  geom_line()
```
...
ggplot(retail_sales_only, aes(x = date, y = value)) +
  geom_line()
```
...
ggplot(unemployment_claims_only, aes(x = date, y = value)) +
  geom_line()
```
...
ggplot(retail_sales, aes(x = date, y = value)) +
  geom_line()
```
...
retail_components_tidy <- retail_components |> 
  ...
  pivot_longer(cols = c(value, trend, season_year, remainder),
